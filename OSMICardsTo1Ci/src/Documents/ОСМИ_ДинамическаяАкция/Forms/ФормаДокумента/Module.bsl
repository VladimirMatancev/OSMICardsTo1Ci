...
&НаКлиенте
Процедура ГруппаПолучателейПриИзменении(Элемент)
	ГруппаПолучателейПриИзмененииНаСервере();
КонецПроцедуры

&НаСервере
Процедура ГруппаПолучателейПриИзмененииНаСервере()
	
	ОбДА = РеквизитФормыВЗначение("Объект");;
	ОбДА.Адресаты.Очистить();
		
	масс = ОбДА.ГруппаПолучателей.ПолучитьОбъект().ПолучитьСписокКарт();
	
	Для каждого карта из масс цикл
		Стр = ОбДА.Адресаты.Добавить();
		Стр.Карта = Карта;
	КонецЦикла;
	
	ЗначениеВРеквизитФормы(ОБДА,"Объект");
